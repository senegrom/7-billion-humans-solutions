...
static void cont_walk(Sim *S, int i, int tx, int ty, bool single, bool flex) {
     ...
     * line at the same speed: a cardinal stride runs 26 frames instead of
     * 21, a rising diagonal 36 instead of 30.  Any southward or lateral
     * approach walks straight in at the plain pace, and a body carrying a
     * cube of its own strides straight too.  Decided at set-off from the
     * same board image that built the route: a cube lifted while the body
     * is moving does not straighten the already chosen path. */
