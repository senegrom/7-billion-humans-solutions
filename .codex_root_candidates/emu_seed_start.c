...
typedef struct { MemKind k; int num, x, y; int ntype; /* CmpKind or -1 */
                 ...
                 bool touched; /* some command has stored here, even one that
                                  found nothing.  A virgin register answers 0
                                  to numeric tests; an EMPTIED one refuses */
...
typedef struct {
    ...
    int  greet_beat;              /* when it did (-1 = never greeted) */
...
typedef enum {
    G_CUBES_ON_GOALS, G_SHREDDED_N, G_SHREDDED_SPLIT, G_ALL_EXITED,   /* v1 */
    ...
    G_CUBES_LINE_ROW, G_CUBES_DIAGONAL, G_WORKERS_ESCAPE, G_PRINT_SHRED_FOREVER,
...
static GoalKind goal_from(const char *g, int *a, int *b) {
    ...
    if (!strcmp(kw,"shredded_split"))      { if (!*b) *b = 50; return G_SHREDDED_SPLIT; }
    ...
    if (!strcmp(kw,"workers_reach_door"))  return G_WORKERS_ESCAPE;
...
typedef struct {
    ...
    int     escape_x, escape_y;  /* G_WORKERS_ESCAPE exit pit */
...
static void sim_reset(Sim *S, Level *L, unsigned seed) {
    ...
    if (getenv("EMU_CUBELOG"))
        for (int i = 0; i < S->nic; i++)
            fprintf(stderr, "[cube] (%d,%d)=%d id=%d\n",
                    S->icx[i], S->icy[i], S->icv[i], S->ic_id[i]);
    ...
    S->escape_x = S->escape_y = -1;
    if (L->win == G_WORKERS_ESCAPE) {
        /* The emergency exit is a PIT, not the room's door -- the door is
         * walled off behind the pits and nobody can reach it.  The numbered
         * cubes spell a trail (1 up, 2 right, 3 down, 4 left) and following it
         * walks its reader off the cubes into one particular pit: that pit is
         * the way out, and every worker has to take it. */
        int cx = S->nic ? S->icx[0] : -1, cy = S->nic ? S->icy[0] : -1;
        for (int steps = 0; steps <= S->nic && cx >= 0; steps++) {
            int idx = -1;
            for (int i = 0; i < S->nic; i++)
                if (S->icx[i] == cx && S->icy[i] == cy) { idx = i; break; }
            if (idx < 0) break;                      /* off the trail: this is it */
            switch (S->icv[idx]) {
                case 1: cy--; break;
                case 2: cx++; break;
                case 3: cy++; break;
            ...
            if (cx < 0 || cy < 0 || cx >= L->w || cy >= L->h) { cx = cy = -1; break; }
        }
        if (cx >= 0 && cy >= 0 && L->terr[cy][cx] == T_HOLE) {
            S->escape_x = cx; S->escape_y = cy;
        }
    }
...
static bool mem_atom_value(Sim *S, Worker *w, const MemAtom *a, int *out) {
    ...
    if (a->wref >= 0 && a->wref < S->nw) {
        /* live hands, same as a remembered person: a grab begun earlier in
         * this frame by an earlier-row worker already counts */
        Worker *other = &S->w[a->wref];
        if (!other->alive || !other->holding) return false;
        *out = other->held;
        return true;
    }
...
static bool operand_value(Sim *S, Worker *w, const Operand *o, int *out) {
    switch (o->kind) {
        ...
        case 2: {
            ...
            /* a virgin register answers 0: a numeric test on a slot nobody
             * ever wrote passes ">= 0" outright.  (The fashion-party cull
             * hinges on it -- the worker whose neighbour probe never ran
             * sails through "mem2 >= 0" and lives.)  A slot a command DID
             * store to, even finding bare floor, refuses the read instead
             * (the backup-day copier leans on "myitem > mem1" staying quiet
             * when its own square held no original). */
            if (m->k == MV_NOTHING) {
                if (!m->touched) { *out = 0; return true; }
                return false;
            }
            ...
            /* a slot that remembers a PERSON asks THEM for a number: their
             * hands answer first, wherever they have wandered off to.  A
             * glance that found them (a foreachdir sample) keeps a second
             * string to its bow -- when the hands are empty it reads what
             * lies on the sampled square NOW.  A remembered nearest-person
             * has no square to fall back on. */
            if (m->k == MV_TILE && m->wref >= 0) {
                /* hands are read live, not from the frame's opening picture:
                 * a lift that began earlier in this very frame already
                 * answers.  Workers act in row order within a frame, so a
                 * later neighbour sees the grab the instant it starts --
                 * one frame of order decides who the fashion cull spares. */
                if (m->wref < S->nw && S->w[m->wref].alive
                    && S->w[m->wref].holding) {
                    *out = S->w[m->wref].held; return true;
                }
                if (m->fedir)
                    return value_at2(S, m->x, m->y, w, false, out);
                return false;
            }
...
static bool cond_true(Sim *S, Cond *c, Worker *w) {
    if (c->rhs_is_type) {
        ...
        if (c->lhs.kind == 1)
            ...
            else if (m->k == MV_TILE && m->wref >= 0) {
                /* the slot remembers a PERSON: the type test follows them,
                 * wherever they have walked since -- it does not go stale
                 * when they leave the sampled square.  A square is a set of
                 * contents, and the person carries that view with them: asked
                 * about a datacube, the slot answers for the floor under
                 * their feet. */
                bool exists = m->wref < S->nw && S->w[m->wref].alive;
                if (c->rhs_type == C_PERSON || c->rhs_type == C_SOMETHING)
                    eq = exists;
                else if (c->rhs_type == C_NOTHING)
                    eq = !exists;
                else if (c->rhs_type == C_DATACUBE && exists)
                    eq = tile_contains(S, seen_tx(S, m->wref),
                                       seen_ty(S, m->wref), w, C_DATACUBE);
                else eq = false;
            }
    ...
    /* A SQUARE with no number on it answers an equality question anyway: it
     * is not showing the number asked about, so `!=` holds and `==` does not.
     * Emergency Escapades' shortest solution is built on it -- its trail
     * follower leans on `se != 3` over bare floor to keep walking, and reading
     * that as "no answer" marches every worker into the nearest pit instead.
     * Ordering questions still need two numbers to compare. */
    if ((!ha && c->lhs.kind == 1) || (!hb && c->rhs.kind == 1)) {
        if (c->op == O_NE) return true;
        if (c->op == O_EQ) return false;
    }
...
static bool level_won(Sim *S) {
    ...
    switch (L->win) {
        ...
        case G_SHREDDED_SPLIT: {
            /* the big numbers go in the shredder on the right and the small
             * ones in the shredder on the left, so a cube fed to the wrong
             * machine spoils the run however many get shredded in total */
            if (S->shredded < L->goal_a) return false;
            ...
            if (leftx == rightx) return true;              /* only one machine */
            for (int e = 0; e < S->nshrev; e++) {
                ShredEv *ev = &S->shrev[e];
                bool big = ev->value >= L->goal_b;
                if (ev->shr_x == rightx && !big) return false;
                if (ev->shr_x == leftx  &&  big) return false;
            }
            return true;
        }
        case G_ALL_EXITED:
        ...
        case G_TUT_PICKUP_DROP:
            /* the exercise is to step down onto the cube, lift it and set it
             * down again, so every cube ends the day on the square it started
             * on.  Reaching down from where you stand lifts and drops it just
             * as often, but leaves it one square from home. */
            if (S->pickups < S->nic || S->drops < S->nic) return false;
            if (floor_cube_count(S) != S->nic) return false;
            for (int i = 0; i < S->nic; i++)
                if (!S->grid[S->icy[i]][S->icx[i]].has_cube) return false;
            return true;
        case G_CUBES_OFFSET: {
            for (int i = 0; i < S->nic; i++) {
        ...
        case G_CUBES_DIAGONAL: {
            ...
            /* the line runs DOWN to the right from the anchor.  A line of the
             * opposite slope reads just as diagonally on the floor, but the
             * game does not accept it: cubes dragged into the rising line sit
             * there forever with the level still open. */
            for (int i = 1; i < n; i++) {
        ...
        case G_WORKERS_ESCAPE:
            /* out through the one pit the cube trail leads to.  The room is
             * full of other pits, and a worker any of those swallows has left
             * the floor without ever finding the emergency exit. */
            if (S->escape_x < 0) return false;
            for (int i = 0; i < S->nw; i++) {
                Worker *w = &S->w[i];
                if (!w->exited || w->exit_x != S->escape_x || w->exit_y != S->escape_y)
                    return false;
            }
            return true;
        ...
        case G_ROYALE_MAX_REMAINS: {
            /* every worker takes the cube in front of them into the contest,
             * so nothing is left lying on the floor: the winners hold theirs
             * and the losers take theirs down the hole. */
            if (floor_cube_count(S)) return false;
            int max = 0;
        ...
        case G_CHAIN_GREET: {
            /* the word travels DOWN THE LINE: everyone but the westmost is
             * greeted, and each waits their turn, so a worker hears it only
             * after the neighbour on their left already has.  Everyone
             * shouting at once greets the whole line but skips the queue. */
            int order[MAXWORKERS], n = 0;
            for (int i = 0; i < S->nw; i++) order[n++] = i;
            for (int i = 1; i < n; i++)          /* sort west to east */
                for (int j = i; j > 0 && S->w[order[j]].x < S->w[order[j-1]].x; j--) {
                    int t = order[j]; order[j] = order[j-1]; order[j-1] = t;
                }
            for (int i = 1; i < n; i++) {
                Worker *cur = &S->w[order[i]], *prev = &S->w[order[i-1]];
                if (!cur->greeted) return false;
                if (i > 1 && cur->greet_beat <= prev->greet_beat) return false;
            }
            return true;
        }
        case G_TRAINING_DAY:
        ...
        case G_SHRED_MAX_PER_GROUP: {
            ...
            for (int g = 0; g < S->ngroups; g++) {
                int ev = 0, took = -1, tx = -1, ty = -1;
                for (int e = 0; e < S->nshrev; e++) {
                    for (int k = 0; k < S->nic; k++)
                        if (S->ic_group[k] == g
                            && S->icx[k] == S->shrev[e].src_x && S->icy[k] == S->shrev[e].src_y) {
                            ev++;
                            took = S->shrev[e].value;
                            tx = S->shrev[e].src_x; ty = S->shrev[e].src_y;
                        }
                }
                /* the cube fed in has to be the biggest number in its group --
                 * as it READS when it goes in.  A worker may write a bigger
                 * number on the one it is carrying and shred that instead;
                 * what is graded is the number on the cube, not the number it
                 * happened to start with. */
                for (int k = 0; k < S->nic && took >= 0; k++) {
                    if (S->ic_group[k] != g) continue;
                    int x = S->icx[k], y = S->icy[k];
                    if (x == tx && y == ty) continue;
                    if (S->grid[y][x].has_cube && S->grid[y][x].cube > took) {
                        if (!g_goal_dbg) return false;
                        fprintf(stderr,
                                "shred_max: group %d fed a %d while (%d,%d) still shows %d\n",
                                g, took, x, y, S->grid[y][x].cube);
                        ok = false;
                    }
                }
                if (ev != 1) {
        ...
        case G_GOODBYE: {
            /* everyone leaves by the convenient hole at the BOTTOM of the room.
             * The room's other pits will swallow a worker just as happily, but
             * one that drops down the wrong one has not left the way it was
             * asked to -- and the goodbye belongs to the last one out, alone. */
            int hx = -1, hy = -1;
            for (int y = 0; y < L->h; y++)
                for (int x = 0; x < L->w; x++)
                    if (L->terr[y][x] == T_HOLE && y > hy) { hx = x; hy = y; }
            int last = -1;
            for (int i = 0; i < S->nw; i++) {
                if (!S->w[i].exited) return false;
                if (hy >= 0 && (S->w[i].exit_x != hx || S->w[i].exit_y != hy))
                    return false;
...
static bool mem_tile(Sim *S, Worker *w, int slot, int *tx, int *ty) {
    ...
    /* a slot that remembers a PERSON from a `nearest` search names the
     * person, not the square they stood on: every use follows them to
     * wherever they have since walked -- read at the square they hold RIGHT
     * NOW, which for someone mid-step is already the square they are
     * stepping into.  A DIRECTIONAL sample is different: it names a SQUARE
     * ("the tile to my south"), and a worker standing there does not turn
     * the step into a chase -- it steps onto the square, swapping if need
     * be.  So only a nearest-person slot follows the body. */
    ...
    switch (ins->op) {
        ...
        case OP_TELL: {
            ...
            for (int j = 0; j < S->nw; j++) {
                ...
                if (covered) {
                    o->heard = MS_EARSHOT;
                    if (!o->greeted) o->greet_beat = S->beat;   /* the FIRST greeting is their turn */
                    o->greeted = true;
 ...
 * hands or to nothing, dropping with empty hands or onto a tile that already
 * has a cube.
...
static void cont_walk(Sim *S, int i, int tx, int ty, bool single, bool flex) {
    ...
    /* A NORTHWARD stride onto a square holding a floor cube swings around
     * the cube's far side -- the body walks a longer line at the same
     * speed: a cardinal stride runs 26 frames instead of 21, a rising
     * diagonal 36 instead of 30.  Any southward or lateral approach walks
     * straight in at the plain pace, and a body carrying a cube of its own
     * strides straight too.  (A southward stride onto a cube DOES take
     * longer in some crowd choreographies -- the sorting-floor carry
     * rounds show ~5 extra frames -- but a plain south-bow rule breaks the
     * counters, whose workers step south onto their digit cubes at the
     * calibrated 21f; the distinguisher is unresolved, so southward stays
     * straight.)  Decided at set-off from the same board image that built
     * the route: a cube lifted while the body is moving does not
     * straighten the already chosen path. */
    w->wcurve = !w->holding && ty < w->y
        && tx >= 0 && ty >= 0 && tx < S->L->w && ty < S->L->h
        && S->grid[ty][tx].has_cube && S->grid[ty][tx].terrain == T_FLOOR;
    if (w->wcurve)
        w->wtot = diag ? 36 : 26;
    if (snap) w->wtot++;
...
static bool cont_glide(Sim *S, Program *P, int i) {
    ...
    if (getenv("EMU_BLKLOG") && occ >= 0 && (S->beat % 100) == 0)
        fprintf(stderr, "[blk] t%d w%d @%d,%d wants %d,%d held by w%d@%d,%d%s%s\n",
                S->beat, i, w->x, w->y, tx, ty, occ,
                S->w[occ].x, S->w[occ].y,
                S->w[occ].done ? " done" : "",
                S->w[occ].alive ? "" : " dead");
...
static void step_dispatch(Sim *S, Program *P, int i, bool *progressed) {
    ...
    Instr *ins = &P->instr[w->pc];
        ...
        if (ins->mem_target >= 0) {
            ...
            /* a step walks up BESIDE a remembered person only when the slot
             * names a person as such -- a `nearest worker` result.  A
             * directional foreach sample names a SQUARE, so a step to it
             * heads for that square and swaps with whoever stands there.
             * A person-slot whose person is gone degrades to the sampled
             * square: there is nobody left to walk up beside. */
            int pref = w->mem[ins->mem_target].wref;
            bool person = pref >= 0 && pref < S->nw
                && !w->mem[ins->mem_target].fedir
                && S->w[pref].alive && !S->w[pref].exited;
            int tx, ty;
            ...
            if (!have
                ...
                /* A command that starts on its remembered square still takes
                 * a moment to not happen.  Do not charge that stand again
                 * when this dispatch is merely completing a journey that the
                 * same step command already walked. */
                else if (w->enroute_pc != w->pc) w->busy = 2;
            ...
            if (!person && (w->mem[ins->mem_target].k == MV_CUBEREF
                 ...
                 * metronome.)  Once that cube has been carried off, the
                 * step walks to the remembered square itself, whatever
                 * else lies on it -- the memory is the square, not the
                 * cube.  A cube the level laid out never blocks. */
                if (w->fresh > 0) w->fresh--;
                w->pc++;
...
static int FQ_FOREACH_BASE = 500;     /* ms of one standard command per sweep */
...
static bool foreach_next(Sim *S, Worker *w, Instr *ins) {
    ...
    if (getenv("EMU_MEMLOG"))
        fprintf(stderr, "[fe] t%d w%d slot%d @%d,%d cube=%d who=%d\n",
                S->beat, (int)(w - S->w), ins->slot + 1, x, y, cube, who);
    /* a square holding both a person and a floor cube samples THE PERSON:
     * the slot follows them for type tests (a walker who leaves is still a
     * worker), while a numeric read asks their hands first and then falls
     * back to whatever lies on the sampled square now */
    if (who >= 0)
        w->mem[ins->slot] = (MemVal){ .k = MV_TILE, .touched = true,
            .x = x, .y = y, .ntype = -1, .wref = who, .fedir = true };
    else if (cube)
        w->mem[ins->slot] = (MemVal){ .k = MV_CUBEREF, .num = cube, .touched = true,
...
static void fq_dispatch(Sim *S, Program *P, int i, int now,
    ...
    switch (ins->op) {
        ...
        case OP_ENDFOR:
            /* No sweep under way (a jumped-into label with an exhausted or
             * absent record) -> fall out of the loop.  Otherwise return to
             * the header, which advances to the next direction or, if the
             * sweep is spent, exits there for free -- so a body jumped into
             * mid-sweep still completes the directions it has left. */
        ...
        case OP_FOREACH: {
            ...
            if (foreach_next(S, w, ins)) {
                ...
                if (ins->ndirs > 0) {
                     ...
                     * only iterations that yield a square pay -- the final
                     * came-up-empty visit leaves for free.  A sweep with an
                     * EMPTY body ends after this single pass: with nothing
                     * to run between header and endfor there is nothing to
                     * come back for. */
                    int n = ins->ndirs;
                    int b = (FQ_FOREACH_BASE + MS_TICK * n - 1) / (MS_TICK * n);
                    fq_push(w, FQ_WAIT, (float)(b > 0 ? b : 1));
                    w->fready = false;
                    if (ins->target == w->pc) *fi = 0;
                }
    ...
    /* a failed item action stops the worker for the error bubble */
    {
        ...
        /* GIVETO probes machines, then workers.  With an item in hand but no
         * eligible target, the real handler jumps through the same 1.5-second
         * error exit as an empty-handed give (0x45ca1b..24,
         * error_cant_give_item_to_nothing).  Keep an already-bound shredder
         * errand alive while its worker walks away from the original probe. */
        else if (ins->op == OP_GIVETO && w->holding && ins->mem_target < 0) {
            int mx, my;
            bool found = w->err_pc == w->pc && w->errx >= 0
                      && S->grid[w->erry][w->errx].terrain == T_SHREDDER;
            if (!found) found = machine_target(S, w, ins, &mx, &my);
            for (int k = 0; k < ins->ndirs && !found; k++) {
                int nx = w->x + DX[ins->dirs[k]], ny = w->y + DY[ins->dirs[k]];
                if (nx < 0 || ny < 0 || nx >= S->L->w || ny >= S->L->h) continue;
                int j = worker_at(S, nx, ny, i);
                found = j >= 0 && !S->w[j].holding;
            }
            if (!found) err = true;
        }
    ...
    switch (ins->op) {
        case OP_IF:
            ...
            if (!if_looks(ins)) {
                if (w->fresh > 0) w->fresh--;
                bool taken = if_true(S, ins, w);
                if (taken) w->pc++;
                else w->pc = ins->target +
                         (P->instr[ins->target].op == OP_ELSE ? 1 : 0);
                /* deciding claims the frame whether the test passes or not:
                 * a chain of headless conditions advances one per frame.  A
                 * nearest right after a satisfied list-test still settles
                 * its pointer inside the decision's frame -- only the
                 * command after it waits for the next one. */
                if (taken && if_inline_multimem_numeric(w, ins)
                    && w->pc < P->n && P->instr[w->pc].op == OP_ASSIGN
                    && P->instr[w->pc].akind == 0) {
                    exec_assign(S, w, &P->instr[w->pc]);
                    w->pc++;
                }
                w->busy = 1;
                *progressed = true; return;
        ...
        case OP_ASSIGN:
            ...
            if (ins->akind == 2) {
                 ...
                 * command up is part of its length.  A pointed LEFT operand
                 * spends one normal look before the arithmetic; a pointed
                 * right operand adds no separate look of its own. */
                w->calc_pc = w->pc;
                w->calc_a = calc_snapshot(S, w, &ins->op1);
                w->calc_b = calc_snapshot(S, w, &ins->op2);
                int scan = ins->op1.kind == 1
...
/* The real game aborts a run after 1,400 seconds.  One simulation frame is
 * 16 ms, so later completions are not valid solution wins. */
enum { GAME_RUN_FRAME_CAP = 1400 * 1000 / 16 };

static bool run_frame(Sim *S, Program *P, int *out_rounds) {
    static int cap = 0;
    if (!cap) {
        const char *e = getenv("EMU_CAP");
        cap = e ? atoi(e) : GAME_RUN_FRAME_CAP;
        if (cap < 1000) cap = GAME_RUN_FRAME_CAP;
    }
    ...
    while (now < cap) {
        ...
        /* The office clock allows 1400 seconds.  A run still going then is
         * called off as having taken too many steps, so a goal reached after
         * that moment does not count and the run can stop: nothing that
         * happens later can ever be a win. */
        if (now > 87500) { *out_rounds = now; return false; }
...
int main(int argc, char **argv) {
    ...
    const char *seed_env = getenv("EMU_SEED_START");
    if (seed_env && *seed_env) {
        char *seed_end = NULL;
        unsigned long parsed_seed = strtoul(seed_env, &seed_end, 10);
        if (seed_end != seed_env && !*seed_end && parsed_seed >= 1 && parsed_seed <= UINT_MAX)
            seed_start = (unsigned)parsed_seed;
    }
    ...
    for (int t = 0; t < trials; t++) {
        sim_reset(&S, &L, seed_start + (unsigned)t);
        int rounds = 0;
    ...
    /* A rare-luck solution can be genuinely winnable yet miss every stock
     * trial (seven dice landing a perfect permutation come up about once in
     * 160 worlds).  When all stock trials lost and lost QUICKLY, keep
     * sampling fresh worlds under a bounded frame budget so "can this win
     * at all" gets a fair answer.  Slow or capped losses eat the budget in
     * a couple of goes and stop; an explicit trial count is left alone. */
    if (!wins && argc != 4 && (L.has_random || prog_random)) {
        /* the budget scales with the per-trial frame cap so the number of
         * EXTRA WORLDS sampled stays the same however patient each trial
         * is allowed to be -- a witness found under a small cap is still
         * found under a large one */
        const char *ce = getenv("EMU_CAP");
        long capv = ce ? atol(ce) : GAME_RUN_FRAME_CAP;
        if (capv < 1000) capv = GAME_RUN_FRAME_CAP;
        long budget = 10 * capv;
        if (budget < 600000) budget = 600000;
        long extra = 0;
        for (int t = trials; extra < budget && t < 20000; t++) {
            sim_reset(&S, &L, (unsigned)(t + 1));
            int rounds = 0;
            bool won = run(&S, &P, &rounds);
            extra += rounds > 0 ? rounds : 1;
            trials = t + 1;
            if (won) {
                wins++;
                min_r = max_r = rounds; sum_r = rounds;
                int sp = (int)((S.win_ms * 16 + 999) / 1000);
                min_sp = max_sp = sp; sum_sp = sp;
                sum_items = S.st_items;
                break;
            }
        }
